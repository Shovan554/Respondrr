...
### alerts
System and health alerts for patients with acknowledgment tracking.
...
### resq_alerts
Emergency alerts from Presage ResQ wearable device integration.

| Column | Type | Nullable | Default |
|--------|------|----------|---------|
| id | bigint | NO | generated by default as identity |
| event_id | uuid | NO | - (unique) |
| patient_user_id | uuid | YES | NULL |
| patient_email | text | NO | - |
| trauma_type | text | NO | 'unknown' |
| status | text | NO | 'triggered' |
| severity | text | NO | 'critical' |
| trigger_g | double precision | YES | NULL |
| countdown_seconds | integer | NO | 10 |
| source | text | NO | 'simulator' |
| heart_rate | double precision | YES | NULL |
| spo2 | double precision | YES | NULL |
| location_lat | double precision | NO | - |
| location_lng | double precision | NO | - |
| location_accuracy_m | double precision | YES | NULL |
| medical_id | jsonb | NO | '{}'::jsonb |
| request_hash | text | NO | - |
| signature_key_id | text | NO | 'v1' |
| presage_data | jsonb | YES | NULL |
| perfusion_index | double precision | YES | NULL |
| shock_flag | text | YES | NULL |
| created_at | timestamp with time zone | NO | now() |
| updated_at | timestamp with time zone | NO | now() |

**Primary Key:** id  
**Foreign Key:** patient_user_id (references profiles.id)  
**Unique Index:** event_id  
**Check Constraints:**
  - status IN ('triggered', 'acknowledged', 'resolved', 'cancelled')
  - severity IN ('low', 'medium', 'high', 'critical')  
**Status Values:**
  - 'triggered' - alert has been triggered by device
...
### resq_alert_nonces
Nonce tracking for ResQ alert request validation and security.
...
| key_id | text | NO | 'v1' |
| request_hash | text | NO | - |
| request_ts | bigint | NO | - |
| created_at | timestamp with time zone | NO | now() |

**Primary Key:** nonce  
**Purpose:** Prevents replay attacks on ResQ alert endpoints

---

### emergency_check_queue
Queue for processing emergency health checks asynchronously.

| Column | Type | Nullable | Default |
|--------|------|----------|---------|
| id | bigint | NO | generated by default as identity |
| email | text | NO | - |
| metric_source | text | NO | - |
...
**Primary Key:** id  
**Check Constraint:** metric_source IN ('realtime', 'aggregated')  
**Status Values:**
  - 'pending' - waiting to be processed
  - 'processing' - currently being processed
  - 'completed' - successfully processed
  - 'failed' - processing failed  
**Purpose:** Decouples emergency detection from API requests for better performance

---

### automation_calls
Automated voice calls and guidance during emergencies via Twilio.

| Column | Type | Nullable | Default |
|--------|------|----------|---------|
| id | uuid | NO | gen_random_uuid() |
| emergency_id | uuid | NO | - |
| patient_id | uuid | NO | - |
| patient_name | text | NO | - |
| audio_url | text | NO | - |
| twilio_call_sid | text | YES | NULL |
| status | text | NO | 'initiated' |
| created_at | timestamp with time zone | NO | now() |
| updated_at | timestamp with time zone | YES | now() |

**Primary Key:** id  
**Foreign Keys:** emergency_id (references emergencies.id), patient_id (references profiles.id)  
**Check Constraint:** status IN ('initiated', 'connecting', 'in_progress', 'completed', 'failed')  
**Status Values:**
  - 'initiated' - call initiated
  - 'connecting' - ringing/attempting connection
  - 'in_progress' - call active
  - 'completed' - call completed successfully
  - 'failed' - call failed  
**Purpose:** Provides automated voice guidance and instructions during medical emergencies
...
- **alerts:** System alerts linked to patients and emergencies
- **resq_alerts:** Emergency alerts from Presage ResQ devices linked to patients
- **resq_alert_nonces:** Nonce tracking for ResQ alert request validation
- **emergency_check_queue:** Async task queue for processing emergency health checks
- **automation_calls:** Automated Twilio calls tied to emergency events
